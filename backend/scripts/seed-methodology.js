...
- total EXACTEMENT 20 slides, page de titre comprise (volume imposé, ni plus ni moins) : 1 page de titre + 19 slides de contenu, conclusion incluse. Ce volume permet une slide distincte par cas d'entreprise.`;
...
const STEP_SCHEMAS = [
  ...
  },
  {
    stepKey: 'probleme',
    jsonSchemaDescription: [
      'PASSE A — CONTRAT MÉTIER. Tu ne produis PAS de slides ici : tu produis le contrat de fond qui sera validé par l\'étudiant, puis découpé en slides à la Passe B.',
      'Objet JSON avec les clés suivantes (toutes en camelCase) :',
      '- "sujet" : chaîne — le sujet tel quel, sans le reformuler.',
      '- "motsCles" : tableau d\'objets { "mot": chaîne, "definition": chaîne } — les 4 à 6 mots-clés du sujet. "definition" tient en UNE ligne métier : ce que le mot change concrètement pour une entreprise, pas une définition de dictionnaire.',
      '- "contexte" : chaîne — 3 à 5 faits d\'actualité apportant chacun un CHIFFRE et sa SOURCE explicite (organisme + année). Aucun chiffre sans source : un chiffre non sourcé est un chiffre inventé.',
      '- "tension" : chaîne — la friction réelle qui découle des mots-clés ET du contexte : ce que l\'entreprise doit arbitrer, ce qu\'elle gagne ou perd. C\'est une phrase de travail interne, elle n\'est PAS affichée telle quelle sur une slide.',
      '- "problematique" : chaîne — UNE seule question, qui traduit le sujet large en question précise. Elle commence par « Comment », « En quoi », « Dans quelle mesure », « Quelles conditions », « Par quels leviers », « Quelle place » ou « Sur quels leviers ». Elle nomme la contrainte (coût, compétence, dette, conformité, dépendance, délai, taille d\'entreprise…) et au moins un mot-clé du sujet.',
      '- "justificationProbleme" : chaîne — pourquoi c\'est un problème d\'entreprise AUJOURD\'HUI, en s\'appuyant sur le contexte chiffré.',
      '- "limitesExistant" : tableau de chaînes — ce qui existe déjà (pratiques, normes, outils) et POURQUOI ça ne suffit pas face à CETTE tension. Chaque entrée est un constat de limite, pas une description de solution.',
      '- "preconisations" : tableau d\'objets { "action": chaîne, "cible": chaîne, "moment": chaîne } — les actions qui RÉPONDENT à la question posée. "action" réutilise le vocabulaire de la tension et de la question. "cible" = PME | ETI | grand groupe. "moment" = avant | pendant | après. Posture consultant : des décisions actionnables, jamais des normes listées (ITIL, NIST, ISO) sans prise de position.',
      '- "casEntreprises" : tableau d\'objets { "nom": chaîne, "chiffre": chaîne, "angle": chaîne, "issue": chaîne, "source": chaîne } — 2 à 4 entreprises réelles et connues. "angle" dit en quoi le cas prouve LA tension de ce sujet. "issue" = "succes" | "echec". Au moins UN cas d\'échec. "source" identifie précisément d\'où vient le cas. Un cas qui n\'illustre pas la tension est à écarter.',
      '- "ligneDirectrice" : chaîne — la phrase « fil rouge » de toute la présentation, dérivée de la problématique. Elle est réutilisée et renforcée à toutes les étapes suivantes.',
      '- "ouverture" : chaîne — la question prospective finale, explicitement SANS réponse.',
      'CE QU\'UNE PROBLÉMATIQUE N\'EST PAS : (1) ce n\'est pas le sujet recopié ou paraphrasé — reprendre l\'intitulé en le retournant ou en ajoutant un point d\'interrogation est éliminatoire ; (2) ce n\'est pas une question fermée oui/non, ni une tournure « Faut-il… ? », « Doit-on… ? », « Est-ce souhaitable… ? » qui appelle un débat Pour/Contre ; (3) ce n\'est pas un vœu : « comment optimiser / améliorer / renforcer X » sans nommer la moindre contrainte ne pose aucun problème d\'entreprise ; (4) elle n\'est pas plus large que le sujet.',
      'DOUBLE EXIGENCE DE FOND : la question soulève un PROBLÈME RÉEL (friction documentée que des entreprises rencontrent effectivement) ET appelle une SOLUTION APPORTABLE (les préconisations y répondent concrètement). Le test : si on retire la question, les préconisations doivent perdre leur raison d\'être. Sinon, la question est trop molle.',
      'Longueur : la "problematique" tient en UNE phrase de 35 mots maximum (240 caractères), point d\'interrogation inclus. C\'est un fil rouge que l\'étudiant doit pouvoir redire de mémoire.',
